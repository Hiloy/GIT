		...
		$.ajax({
			...
			success: function (json) {
				...
				if (json['success']) {
					...
					$('#wishlistAlert').html(json['success']).fadeIn();
			
					setTimeout(function() {
							$('#wishlistAlert').fadeOut();
					}, 5000);  // 3 секунды, измените значение, если нужно
	...
	'add': function (product_id) {
		$.ajax({
			...
			success: function (json) {
				...
				if (json['success']) {
					let html = `<div class="modal fade" id="compareModal" tabindex="-1" aria-labelledby="compareModal" aria-hidden="true">
